...
async def main():
    ...
    if st_u != 200 or st_o != 200:
        print("Login failed. Resetting passwords in DB...")
        engine = create_async_engine("postgresql+asyncpg://postgres:raghav@localhost:5432/sih_db")
        async_session = sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)
        async with async_session() as db:
            result = await db.execute(select(User).where(User.email == u_email))
            u_user = result.scalars().first()
            if u_user: u_user.hashed_password = get_password_hash(u_pass)
            
            result = await db.execute(select(User).where(User.email == o_email))
            o_user = result.scalars().first()
            if o_user: o_user.hashed_password = get_password_hash(o_pass)
            
            await db.commit()
        print("Passwords reset in DB.")
        st_u, res_u = login(u_email, u_pass)
        st_o, res_o = login(o_email, o_pass)
    ...
    if st_u == 200:
        ...
        print(f"USER /users/me: {'PASS' if st_ume == 200 else 'FAIL'}")
    ...
    if st_o == 200:
        ...
        print(f"OFFICER /users/me: {'PASS' if st_ome == 200 else 'FAIL'}")
